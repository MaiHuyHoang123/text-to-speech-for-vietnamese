...
app = Flask(__name__)
session = ort.InferenceSession("./finetuning_pretrained_vi_female_epoch=4785.onnx")  # Thay bằng đường dẫn tới mô hình ONNX của bạn

@app.route('/', methods=['GET', 'POST'])
def index():
    if request.method == 'POST':
        text = request.form['text']
        inputs = {"text": text}  # Điều chỉnh đầu vào theo mô hình của bạn
